...
// test
    ...
    steps {
        script {
            ...
            // Note: The loop for curl/sleep is trickier in pure bat, you might need a separate bat script or PowerShell
            // For simplicity, let's assume `curl` is in your PATH. If not, use full path to curl.exe
            bat '''
            ...
            // ... (writeFile for sonar-project.properties - keep as is, it's groovy) ...
            ...
            """
            // Note: No withSonarQubeEnv wrapper if you are entirely using docker-scanner
        }
    }
    post {
        always {
            // ... (archiveArtifacts - keep as is) ...
...
// Updated Quality Gate stage for Docker (assuming you want to query directly)
stage('Quality Gate') {
    agent { label 'testing' }
    steps {
        timeout(time: 10, unit: 'MINUTES') {
            script {
                try {
                    echo "Checking Quality Gate via Docker (direct curl)..."

                    // Get the latest task ID from SonarQube
                    def taskId = bat(
                        script: '''
                            curl -s "http://localhost:9000/api/ce/activity?component=${SONAR_PROJECT_KEY}&status=SUCCESS,FAILED,CANCELED" | findstr /R /C:"\"id\":\"[^\"]*\""
                        ''', // findstr for Windows grep
                        returnStdout: true
                    ).trim()
                    ...
                    def qualityGateStatus = bat(
                        script: '''
                            curl -s "http://localhost:9000/api/qualitygates/project_status?projectKey=${SONAR_PROJECT_KEY}" | findstr /R /C:"\"status\":\"[^\"]*\""
                        ''', // findstr for Windows grep
                        returnStdout: true
                    ).trim()
