                ...
                timeout(time: 10, unit: 'MINUTES') {
                    script {
                        try {
                            ...
                            def taskId = sh(
                                script: '''
                                    curl -s "http://localhost:9000/api/ce/activity?component=${SONAR_PROJECT_KEY}&status=SUCCESS,FAILED,CANCELED" | grep -o '"id":"[^"]*"'
                                ''',
                                returnStdout: true
                            ).trim()
                            ...
                            def qualityGateStatus = sh(
                                script: '''
                                    curl -s "http://localhost:9000/api/qualitygates/project_status?projectKey=${SONAR_PROJECT_KEY}" | grep -o '"status":"[^"]*"'
                                ''',
                                returnStdout: true
                            ).trim()
